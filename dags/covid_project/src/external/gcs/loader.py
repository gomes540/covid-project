...
class GCSLoader:
    def __init__(
        self, *, project_id: str, csv_data_dict: dict, credentials: str
    ) -> None:
    ...
    def _insert_day_in_filename_as_dict_keys(self, csv_dict: dict) -> str:
        ...
        full_csv_data_dict = {
            f"{filename_without_extension}-on-{day}.csv": daily_csv_data
            for (day, daily_csv_data) in csv_dict.items()
        }
    ...
    def load_data(self) -> list:
        ...
        for filename, daily_data in filename_and_data_dict.items():
            ...
            blob.upload_from_string(data=daily_data, content_type=self.content_type)
            logging.info(
